...
当前 [bash.py](../../code/agent/tools/bash.py) 直接调用 `subprocess.run()`，能执行普通命令，但仍是原型实现：

1. 没有 `argument_parser`。空命令、非字符串命令、非法 timeout、未知字段都会越过参数准备阶段，可能在执行时变成笼统的 `TOOL_EXECUTION_FAILED`。
2. timeout 可省略，含义是无限等待。开发服务器、`tail -f`、交互式程序或挂起命令会长期占住 Agent Runtime。
...
8. 现有 `BASH_EXECUTE` 的权限请求没有资源路径。当前只能按“是否执行 Bash”做整体授权，不能可靠判断 Shell 命令访问了哪些目录。
...
- [ ] Bash 参数、权限准备、超时、输出截断和启动失败测试通过。
- [ ] 现有权限模式测试及 Tool 结果编码测试通过。
- [ ] `pytest -q` 通过后提交“完成 Bash 一次性命令优化”。
...
- [ ] Tool Schema、参数解析器、结果结构和描述只表达最终契约。
- [ ] 完整测试、编译检查和 `git diff --check` 通过。
- [ ] 计划文档准确说明：没有 OS 级沙箱，就不宣称拥有完整 Shell 文件访问控制。
...
- [ ] `bash` 参数在权限检查前严格解析，且永不无限等待。
- [ ] Bash stdout/stderr 的内存占用与模型返回内容都有本地硬上限。
- [ ] timeout、启动失败、工作目录失效和编码问题均返回稳定错误码。
- [ ] 超时和 stop 能处理目标进程组/进程树，而非只终止 Shell 父进程。
- [ ] `process` 能启动、查询、分页读取日志、有限等待和停止 Session 所属进程。
- [ ] 持续服务不会阻塞 Agent Loop，也不会因取消当前消息被误停止。
- [ ] Session 关闭或应用退出时清理后台进程；应用重启不恢复旧 PID。
- [ ] 权限模式和 Session 授权规则覆盖 Bash/Process Start；状态、日志和停止不能跨 Session。
- [ ] 完整测试、编译检查和 `git diff --check` 通过。
