...
class ContextRestoreContractTests(unittest.TestCase):
    ...
    def test_context_exposes_session_id_without_execution_context(self):
        """Context 只公开 Session 身份，不要求完整的工具执行环境。"""
        context = Context(
            summarizer_llm=FakeLLM(),
            session_id="session_independent",
        )

        self.assertEqual(context.session_id, "session_independent")
        self.assertFalse(hasattr(context, "sid"))

    def test_restore_replaces_current_messages_without_legacy_apis(self):
        """恢复只使用 restore，兼容入口不能继续暴露。"""
        context = Context(
            FakeLLM(),
            session_id="session_old",
        )
        context.append_user_message("旧消息")
        ...
        for method_name in ("append_msg", "get_msg", "load_history"):
